...
def run_optical_flow(vid_item, dev_id=0):
    ...
    # unzip img flow x y
    os.system("unzip -q  {}.zip -d {}".format(quote(flow_x_path), quote(flow_x_path)))
    os.system("unzip -q  {}.zip -d {}".format(quote(flow_y_path), quote(flow_y_path)))
    os.system("unzip -q  {}.zip -d {}".format(quote(image_path), quote(image_path)))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ext", type=str, default="webm", choices=["avi", "mp4", "mkv", "webm"], help="video file extensions"
    )
